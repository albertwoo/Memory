...
type ``Get memory max count by day with filter handler``(memoryDb: MemoryDbContext) =

    interface IRequestHandler<``Get memory max count by day with filter``, int> with
